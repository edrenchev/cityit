...
class StaticPageRepository extends EntityRepository {
    ...
    public function count($filterData, $languages, $searchModel) {
        ...
        $queryBuilder->select('COUNT(t.id) as cnt')
            ->from(\Admin\Entity\StaticPage::class, 't')
            ->leftJoin(\Admin\Entity\StaticPageLng::class, 'en_GB', \Doctrine\ORM\Query\Expr\Join::WITH, "t.id=en_GB.mid AND en_GB.lng='en_GB'")
            ->leftJoin(\Admin\Entity\StaticPageLng::class, 'bg_BG', \Doctrine\ORM\Query\Expr\Join::WITH, "t.id=bg_BG.mid AND bg_BG.lng='bg_BG'");
...
//        var_dump($result->getSql());
    ...
    public function getMenuTree() {
        $entityManager = $this->getEntityManager();
        $queryBuilder = $entityManager->createQueryBuilder();
        return $queryBuilder->select(array('t'))->from(\Admin\Entity\StaticPage::class, 't')->addOrderBy('t.ord', 'ASC')->getQuery()->getResult();
    }
